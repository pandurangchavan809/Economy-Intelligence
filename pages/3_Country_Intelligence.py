...
import streamlit as st
from datetime import datetime, UTC # Fixed deprecation by using UTC
from streamlit_autorefresh import st_autorefresh
...
# ---------------- CONFIG ----------------
st.set_page_config(layout="wide")
# We keep st_autorefresh for secondary compatibility, but fragment handles the smooth UI
st_autorefresh(interval=1000, key="country_live_refresh")

# Fixed: Using timezone-aware UTC to stop log warnings
CURRENT_YEAR = datetime.now(UTC).year
BASE_YEAR = CURRENT_YEAR - 1
...
# This fragment ensures ONLY the live counters re-render, preventing "blurry" screen flickers.
@st.fragment(run_every="1s")
def render_live_section(gdp, gdp_year, real_growth, inflation, pop_val, pop_year, pop_growth_rate):
    # Live Population Calculation
    if pop_val:
        pop_base_year = (pop_year or BASE_YEAR) + 1
        live_pop = live_population_value(pop_val, pop_growth_rate or 0.0, pop_base_year)
        population_display = format_number(int(live_pop))
    ...
    # Live GDP Calculation
    country_gdp_usd = None
    if gdp is not None:
        try: country_gdp_usd = float(gdp)
        except: country_gdp_usd = None
...
# Call the fragment
render_live_section(gdp, gdp_year, real_growth, inflation, pop_val, pop_year, pop_growth_rate)

# --- Remaining Static Indicators (These do not flicker)
st.markdown("---")
k5, k6, k7, k8 = st.columns([2,2,2,2])
k5.metric("Real GDP growth", format_percent(real_growth) if real_growth is not None else "Not available")
k6.metric("Inflation", format_percent(inflation) if inflation is not None else "Not available")
...
k7.metric("Nominal growth (est.)", format_percent(nom_val) if nom_val is not None else "Not available")
k8.metric("Base GDP (stored)", f"${(float(gdp or 0) / 1e12):,.2f} T" if gdp else "Not available")

# trade, military, debt (Static data)
trade = get_trade_for_country(country_id)
if trade:
    ex = trade.get("exports_usd"); im = trade.get("imports_usd"); tb = trade.get("trade_balance_usd")
    ex_disp = f"${float(ex)/1e12:,.2f} T" if ex is not None else "Not available"
...
mil_display = "Not available"
if military_spending:
    try: mil_display = f"${float(military_spending)/1e9:,.2f} B"
    except: mil_display = "Not available"
elif table_has_column("country_military_share", "military_spending"):
    dfm = fetch_df("SELECT military_spending FROM country_military_share WHERE country=%s ORDER BY year DESC LIMIT 1", (country_name,))
    if not dfm.empty: mil_display = f"${float(dfm.iloc[0,0])/1e9:,.2f} B"
...
st.markdown(
    f"""
    **Sources:** IMF WEO · World Bank · country ETL.  
    **Disclaimer:** Live values use the stored base-year snapshot and pro-rata nominal growth from Jan 1 of base-year (stored_year + 1). Exchange-rate conversions come from the ETL snapshot.
    """
)