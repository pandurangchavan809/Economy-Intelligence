...
# ---------------- CONFIG ----------------
st.set_page_config(layout="wide")
# refresh every 1s for the fragment to pick up the pulse
st_autorefresh(interval=1000, key="country_live_refresh")
...
def get_pct_shares(country_id, iso3, continent_code, year):
    pct_cont = None
    pct_world = None
    # try country_continent_gdp_share
    if table_has_column("country_continent_gdp_share", "pct_of_continent"):
        if table_has_column("country_continent_gdp_share", "iso3") and iso3:
            df = fetch_df("SELECT pct_of_continent FROM country_continent_gdp_share WHERE iso3=%s AND year=%s LIMIT 1", (iso3, year))
            if not df.empty:
                pct_cont = float(df.iloc[0,0])
        if pct_cont is None:
            name_df = fetch_df("SELECT name FROM countries WHERE country_id=%s LIMIT 1", (country_id,))
            if not name_df.empty:
                df = fetch_df("SELECT pct_of_continent FROM country_continent_gdp_share WHERE country=%s AND year=%s LIMIT 1", (name_df.iloc[0,0], year))
                if not df.empty:
                    pct_cont = float(df.iloc[0,0])
    # try world share view
    if table_has_column("country_world_gdp_share", "pct_of_world"):
        ...
        if pct_world is None:
            name_df = fetch_df("SELECT name FROM countries WHERE country_id=%s LIMIT 1", (country_id,))
            if not name_df.empty:
                df = fetch_df("SELECT pct_of_world FROM country_world_gdp_share WHERE country=%s AND year=%s LIMIT 1", (name_df.iloc[0,0], year))
                if not df.empty:
                    pct_world = float(df.iloc[0,0])
    # fallback compute using economic_indicators.gdp and continent/world aggregates
    if pct_cont is None or pct_world is None:
...
# Find default index for India (IND)
default_idx = 0
...
k5.metric("Real GDP growth", format_percent(real_growth) if real_growth is not None else "Not available")
k6.metric("Inflation", format_percent(inflation) if inflation is not None else "Not available")
nom_val = (real_growth or 0.0) + (inflation or 0.0) if (real_growth is not None or inflation is not None) else None
k7.metric("Nominal growth (est.)", format_percent(nom_val) if nom_val is not None else "Not available")
k8.metric("Base GDP (stored)", f"${(float(gdp_val or 0) / 1e12):,.2f} T" if gdp_val else "Not available")
...
t1, t2, t3 = st.columns(3)
if trade:
    ex = trade.get("exports_usd")
    im = trade.get("imports_usd")
    tb = trade.get("trade_balance_usd")
    ...
    t1.metric("Exports (latest)", "Not available")
    t2.metric("Imports (latest)", "Not available")
    t3.metric("Trade balance", "Not available")
...
o1.metric("Military spending (latest)", mil_display)
o2.metric("Debt-to-GDP", format_percent(debt_gdp) if debt_gdp is not None else "Not available")
o3.metric("Exchange rate (local per USD)", f"{exchange_rate}" if exchange_rate else "Not available")
...
st.markdown(
    f"""
    **Sources:** IMF WEO · World Bank · country ETL.  
    **Disclaimer:** Live values use the stored base-year snapshot and pro-rata nominal growth from Jan 1 of base-year (stored_year + 1).
    """
)